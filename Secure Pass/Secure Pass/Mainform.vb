...
Public Class MainForm
    ...
    Private Sub MainForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ' Call the method to populate the DataGridView with account names
        PopulateAccountNames()
    End Sub

    Private Sub PopulateAccountNames()
        Try
            ' Retrieve account names from the database for the current user
            Dim accountNames As List(Of String) = dataAccess.GetAccountNames(userID)

            ' Clear any existing rows in the DataGridView
            DataGridView1.Rows.Clear()

            ' Add each account name to the DataGridView
            For Each accountName As String In accountNames
                DataGridView1.Rows.Add(accountName)
            Next
        Catch ex As Exception
            ' Handle any exceptions that may occur while populating the DataGridView
            MessageBox.Show($"Error populating account names: {ex.Message}")
        End Try
    End Sub
